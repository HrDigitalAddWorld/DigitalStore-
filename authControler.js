...
const loginUser = async (req, res) => {
  try {
    ...
    let user =
      ...
      (await Admin.findOne({ email: emailOrMobile }).select("+password"));
...
bcrypt.hash(passwordToHash, 10).then((hash) => {
  console.log("Correct Hashed Password:", hash);
});
    ...
    res.status(200).json({ success: true, token, role: user.role,id: user._id,email: user.businessEmail || user.email, message: "Login successful!" });
  } catch (error) {
